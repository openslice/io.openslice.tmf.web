...
export class EditResourcePoolComponent implements OnInit {
  ...
  filteredResourceRefs$: Observable<string | ResourceRef[]>
  ...
  newResourcePool: boolean = false
    ...
    ngOnInit() {
      ...
      if ( this.activatedRoute.snapshot.params.id ) 
        ...
        else { this.newResourcePool = true }
    ...
    retrieveResourcePool() {
      console.info("resourcePoolID = " + this.resourcePoolID);
      this.resourcePoolService.retrieveResourcePool ({ id: this.resourcePoolID }).subscribe(
        ...
        () => {
          if (this.resourcePool) {
            ...
            console.info("this.resourcePool = " + this.resourcePool);
            ...
            this.filteredResourceRefs$ = this.relatedResourcesFilterCtrl.valueChanges.pipe(
              startWith(null),
              map((value: null | string) => value ? (value) :  this.resourcePool.capacity.resources.slice())
            )
          } else {
    ...
    submitDialog() {
      ...
      let updatedRP: ResourcePool
        ...
        if (this.newResourcePool) {
          this.resourcePoolService.createResourcePool( params ).subscribe(
            ...
            const paramsPatch: ResourcePoolService.PatchResourcePoolParams = {
                id: this.resourcePoolID,
                name: this.editForm.value.name,
                body: updateObj,     
            }
          this.resourcePoolService.patchResourcePool( paramsPatch ).subscribe(
            data => { updatedRP = data },
            error => console.error(error),
            () => { 
      ...
      openAssignResourceRelationshipDialog() {
        ...
        dialogRef.afterClosed().subscribe (
          result => { 
            if (result) { 
              this.toast.success("Resource Pool capacity resources list was successfully updated")
              this.retrieveResourcePool() 
            }
          }
        )
